 ...
 *
 * @author Mark Ling and Harry Lian
 *
 */
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        //creates sequence for children of rep
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //creates root node as statement label
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        //casts s to a statement 2. Need statement2 not statement
        Statement2 temp = (Statement2) s;
        //creates a new sequence for children of new root
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //disassembles rep's children and gets root to reassemble later
        StatementLabel dis = this.rep.disassemble(children);
        //adds to sequence
        children.add(pos, temp.rep);
        //clears temp
        temp.createNewRep();
        //reassembles rep with added children
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        //gets the children
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //disassembles children and gets the statement label
        StatementLabel dis = this.rep.disassemble(children);
        //removes the children from the sequence
        s.rep = children.remove(pos);
        //reassembles rep without the removed statement
        this.rep.assemble(dis, children);
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        //casts s to a statement 2. Need statement2 not statement
        Statement2 temp1 = (Statement2) s1;
        Statement2 temp2 = (Statement2) s2;
        //gets the children of the rep
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //creates a label with the IF_ELSE kind for the root of the loop
        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);
        //adds temps to the children sequence
        children.add(0, temp1.rep);
        children.add(1, temp2.rep);
        //reassembles this.rep with new children
        this.rep.assemble(label, children);
        //clears temps
        temp1.createNewRep();
        temp2.createNewRep();
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        //gets the children of the rep
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //disassembles this.rep and gets the statement label
        StatementLabel label = this.rep.disassemble(children);
        //removes the roots of the temps
        temp1.rep = children.remove(0);
        temp2.rep = children.remove(0);
        //clears this.rep
        this.createNewRep();
        //returns the label's condition
        return label.condition;
    }
    ...
    public final void assembleWhile(Condition c, Statement s) {
        ...
        //gets the children of the rep
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //creates a label with the while kind for the root of the loop
        StatementLabel label = new StatementLabel(Kind.WHILE, c);
        //casts s to a statement 2. Need statement2 not statement
        Statement2 temp = (Statement2) s;
        //adds s/temp to the sequence at beginning
        children.add(0, temp.rep);
        //reassembles rep with while loop inside
        this.rep.assemble(label, children);
        //clears temp
        temp.createNewRep();
    ...
    public final Condition disassembleWhile(Statement s) {
        ...
        //gets the children of the rep
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //disassembles the children
        StatementLabel dis = this.rep.disassemble(children);
        //casts s to a statement 2. Need statement2 not statement
        Statement2 temp = (Statement2) s;
        //removes first child from sequence
        temp.rep = children.remove(0);
        //resets this.rep
        this.createNewRep();
        //returns the condition of the root that was taken out
    ...
    public final void assembleCall(String inst) {
        ...
        //gets the children of the rep
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //creates a new statement label with kind call
        StatementLabel label = new StatementLabel(Kind.CALL, inst);
        //reassembles the rep with new call
        this.rep.assemble(label, children);
    ...
    public final String disassembleCall() {
        ...
        //gets the children of the rep
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        //disassembles the root of the children and gets the statement label
        StatementLabel dis = this.rep.disassemble(children);
        //resets this.rep
        this.createNewRep();
        //returns the instruction of the root that was taken out
