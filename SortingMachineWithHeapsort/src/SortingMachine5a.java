...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        //This is making sure that i and j are not the same.
        if (i != j) {

            //This allows to swap values at each node.
            T temp = array[i];
            array[i] = array[j];
            array[j] = temp;
        }
    ...
    private static <T> void siftDown(T[] array, int top, int last, Comparator<T> order) {
        ...
        //This is the algorithm for the left and right tree.
        int leftTree = 2 * top + 1;
        int rightTree = 2 * top + 2;

        //This is checking if both tree are a heap.
        if (leftTree <= last && rightTree <= last) {

            //This is comparing the value of the leftTree and the rightTree.
            if (order.compare(array[leftTree], array[rightTree]) < 0) {

                //This is comparing the top node value to the leftTree node value.
                if (order.compare(array[top], array[leftTree]) > 0) {
                    exchangeEntries(array, leftTree, top);
                    siftDown(array, leftTree, last, order);
                }

                //This is checking for the value of the rightTree and leftTree node value.
            } else {
                if (order.compare(array[top], array[rightTree]) > 0) {
                    exchangeEntries(array, rightTree, top);
                    siftDown(array, rightTree, last, order);
                }
            }

            //This is condition if the rightTree is not a heap.
        } else if (leftTree <= last) {
            if (order.compare(array[top], array[leftTree]) > 0) {
                exchangeEntries(array, leftTree, top);
            }
        }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        //This is checking if rightTree is within the array.
        if (rightTree < array.length) {
        ...
        //This is checking if leftTree is within the array
        if (leftTree < array.length) {
            heapify(array, leftTree, order);
        }

        //This will swap the value for the ordering property.
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        //This will add value to the heap.
        while (0 < q.length()) {
            heap[i] = q.dequeue();
            i++;
        }
    ...
    public final T removeFirst() {
        ...
        // remove top element of heap and decrement size
        T top = this.heap[0];
        this.heapSize--;

        // exchange elements and call siftDown to garbage position
    ...
    public final int size() {
        ...
        if (this.isInInsertionMode()) {
            answer = this.entries.length();
        } else {
            answer = this.heapSize;
        }
