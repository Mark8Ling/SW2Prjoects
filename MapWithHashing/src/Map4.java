...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        //This is just a normal condition.
        if (a > 0 && b > 0) {
            result = a % b;

            //This is condition if it is negative.
        } else {
            result = (a % b) + b;
        }
    ...
    private void createNewRep(int hashTableSize) {
        ...
        //Loop to initialize each map object
    ...
    public final void add(K key, V value) {
        ...
        //gets size of ht
        int htl = this.hashTable.length;

        //puts K, V into correct hash bucket
        this.hashTable[mod(khc, htl)].add(key, value);

        //increments size of ht
        this.size++;
    ...
    public final Pair<K, V> remove(K key) {
        ...
        //takes correct pair out of the hash bucket
        Pair<K, V> val = this.hashTable[mod(khc, htl)].remove(key);

        //decrements the size of ht
        this.size--;
    ...
    public final Pair<K, V> removeAny() {
        ...
        //This will try to get the an array with an object.
        while (!(this.hashTable[i].size() > 0)) {
            i++;
        }

        //This will remove an arbitrary value from hashtable.
    ...
    public final V value(K key) {
        ...
        //This uses mod to determine which bucket to put key and value.
        int bucket = mod(key.hashCode(), this.hashTable.length);

        //This will find the value of key.
        V valueOfThis = this.hashTable[bucket].value(key);
    ...
    public final boolean hasKey(K key) {
        ...
        //This sees if the key exists in the hashtable.
        boolean existKey = this.hashTable[bucket].hasKey(key);
